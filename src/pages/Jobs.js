...
const Jobs = () => {
  ...
  const toggleBookmark = (job) => {
    ...
    if (bookmarked.has(jobId)) {
      const updatedBookmarks = storedBookmarks.filter((bookmark) => bookmark.id !== jobId);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {jobs.map((job) => (
              ...
              >
                ...
                <div>
                  <h2 className="text-xl font-semibold text-gray-800 mb-2">Job Name: {job.title}</h2>
                  <div className="flex items-center text-gray-600 mb-2">
                    <FaBriefcase className="h-5 w-5 mr-2 text-indigo-600" />
                    <span className="truncate">Company Name: {job.company_name}</span>
                  </div>
                  <div className="flex items-center text-gray-600 mb-2">
                    <FaMapMarkerAlt className="h-5 w-5 mr-2 text-green-500" />
                    <span className="truncate">Location: {job.job_location_slug}</span>
                  </div>
                  <div className="flex items-center text-gray-600 mb-2">
                    <FaDollarSign className="h-5 w-5 mr-2 text-yellow-500" />
                    <span className="truncate">Salary: {job.salary_max || "Not Disclosed"}</span>
                  </div>
                  <div className="flex items-center text-gray-600 mb-4">
                    <FaPhone className="h-5 w-5 mr-2 text-blue-500" />
                    <span className="truncate">Phone Number: {job.whatsapp_no}</span>
                  </div>
                </div>
                <button
